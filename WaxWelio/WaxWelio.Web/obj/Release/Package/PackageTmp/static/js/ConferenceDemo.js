...
$(document).ready(function () {
    ...
    //if (cookievalue == "" || cookievalue == null) {
        ...
        //    displayName = decodeURIComponent(getQueryStringParameter("displayName", decryptedUriParameters));
        //    meetingId = decodeURIComponent(getQueryStringParameter("meetingId", decryptedUriParameters));
        //meetingId = getParameterByName("meetingId", document.URL);
        //usertype = getParameterByName("userType", document.URL);
        //displayName = getParameterByName("displayName", document.URL);
        //var qs = meetingId + ":" + usertype + ":" + displayName;
        //setCookie("meetingdetails", qs);
        //GetMeetingDetails();
    //}
...
function register_popup(participantEmail, id, name) {
    ...
    if (objExists == null) {
                ...
                '<div id="chatServiceWrap_' + id + '" class="chatServiceWrap"> ' +
                    '<div class="panel panel-primary">' +
                        '<div class="panel-heading" id="accordion">' +
                            '<span class="glyphicon glyphicon-comment"></span> ' + patientName +
                            '<div class="btn-group pull-right">' +
                                '<a type="button" class="btn-default-override btn-xs close-chat-btn" data-toggle="collapse" data-parent="#accordion" href="#collapseOne" id="chatClose_' + id + '" >' +
                    ...
                    '<div class="panel-body">' +
                        ...
                        '<div>' +
                            '<textarea id="message_' + id + '" type="text" placeholder="Type Message here" style="border:1px solid black;width:85%"/></textarea>' +
                            '<button style="margin:0;padding:0;height:50px;width:48px;font-size:15px" id="j-send-msg">Send</button>'
                        '</div>' +
                    '</div>' +
                    '</div>' +
                '</div></div>';
        ...
        $('#chatMinimize_' + id).attr('onclick', 'minimize_popup("' + main_id + '")');
        $('#j-send-msg').click(function (e) {
            var textmessage = $('#message_' + id.replace(/[^\w\s]/gi, '')).html();
            if (textmessage != '') {
                if (participantEmail != '') {
                    SendIMMessages(participantEmail, main_id);
                }
                else {
                    sendMessage(meetingUri, main_id);
                }
            }
            $('#message-history_' + id).animate({ scrollTop: $('#message-history_' + id)[0].scrollHeight }, 200);
        })
        ...
        if (participantEmail != '') {
            $('#sendMessasge_' + id).attr('onclick', 'SendIMMessages("' + participantEmail + '","' + main_id + '")');
            $('#chatClose_' + id).attr('onclick', 'close_multipleChats("' + participantEmail + '", "' + main_id + '")');
        }
        else {
            $('#sendMessasge_' + id).attr('onclick', 'sendMessage("' + meetingUri + '", "' + main_id + '")');
            $('#chatClose_' + id).attr('onclick', 'close_popup("' + meetingUri + '", "' + main_id + '")');
        }
        var olddata = LocalStorage.GetData(appointmentId);
        $.each(olddata, function (i, msg) {
            $('#message-history_' + id).append(msg);
        });
        $('#message-history_' + id).animate({ scrollTop: $('#message-history_' + id)[0].scrollHeight }, 200);
    }
    popups.unshift(id);
...
function display_popups() {
    ...
    for (iii; iii < total_popups; iii++) {
        if (popups[iii] != undefined) {
            var ele = 'chatPopUp_' + popups[iii];
            element = document.getElementById(ele.replace(/[^\w\s]/gi, ''));
            if (element != null) {
...
function minimize_popup(id) {
    for (var iii = 0; iii < popups.length; iii++) {
        if (id == popups[iii]) {
            Array.remove(popups, iii);
            document.getElementById('chatPopUp_' + id.replace(/[^\w\s]/gi, '')).style.display = "none";
            calculate_popups();
            $('#lobbyUsers').show();
            return;
        }
...
function GetMeetingDetails() {
    //var url = baseUrl + 'HealthCare/GetMeetingUri?meetingId=' + meetingId;
    //$.ajax({
    //    url: url,
    //    type: 'GET',
    //    dataType: 'json',
...
$('#btnChatService').click(function () {
    if ($('#multipleChats').children().length > 0) {
        $('#multipleChats').show();
        $('#lobbyUsers').hide();
    } else {
...
window.onbeforeunload = function (e) {
    if (joined) {
        //HangUpCall(meetingUri, function () { });
        $('#loadingVideo').hide();
        var dialogText = 'Conversation will get disconnected. Are you sure you want to continue?';
...
//function updateMeetingEndTime(callback) {
